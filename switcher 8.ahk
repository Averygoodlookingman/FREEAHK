...
q::
click_points(pointsq)
return

w::
click_points(pointsw)
return

e::
click_points(pointse)
return

r::
click_points(pointsr)
return
...
^q::
hoverColor := "Purple"
add_point(pointsq, "Purple")
return

^w::
hoverColor := "White"
add_point(pointsw, "White")
return

^e::
hoverColor := "Black"
add_point(pointse, "Black")
return

^r::
hoverColor := "Teal"
add_point(pointsr, "Teal")
return
...
!q::
clear_points(pointsq)
return

!w::
clear_points(pointsw)
return

!e::
clear_points(pointse)
return

!r::
clear_points(pointsr)
return
...
start_script() {
	...
	ToolTip, Google Nom Scripts for his original scripts`nI removed his links because he released`na new free tier version of this script`nwith 2 color swaps that can only click 2 times`nit used to have 3 with unlimited clicks`nthis one allows 8 colors with unlimited clicks`nEdited by Mynameajeff`nCtrl+1/2/3/4/q/w/e/r Add point`nAlt+1/2/3/4/q/w/e/r Clear points`n1/2/3/4/q/w/e/r Click points`nF6 to exit`nF5 toggle display,0,0
	width:=30
	height:=30
}
